      ...
      <div class="card">
        ...
        <div class="card-body px-0 pb-0">
          ...
          <tr>
            ...
            <td class="text-sm font-weight-bold">{{ $supplier_data->nama_supplier }}</td>
            <td class="text-sm font-weight-bold">{{ $supplier_data->alamat_supplier }}</td>
            <td class="text-sm font-weight-bold">{{ $supplier_data->no_telepon	 }}</td>
            <td class="text-sm">
            <div class="btn-group" role="group" aria-label="action buttons">
              <button data-bs-toggle="modal" data-bs-target="#detailModal-{{ $supplier_data->id_supplier }}"
              class="btn bg-gradient-success btn-icon-only" type="button" data-bs-toggle="tooltip"
              data-bs-placement="top" title="Detail">
              <i class="fas fa-search"></i>
              </button>
              <button data-bs-toggle="modal" data-bs-target="#updateModal-{{ $supplier_data->id_supplier }}"
              class="btn bg-gradient-primary btn-icon-only" type="button" data-bs-toggle="tooltip"
              data-bs-placement="top" title="Update">
              <i class="fas fa-pen"></i>
              </button>
              <button data-bs-toggle="modal" data-bs-target="#deleteModal-{{ $supplier_data->id_supplier }}"
          ...
          @include('admin.supplier.supplier-detail', ['supplier_data' => $supplier_data])
          @include('admin.supplier.supplier-update', ['supplier_data' => $supplier_data])
          @include('admin.supplier.supplier-delete', ['supplier_data' => $supplier_data])
        @endforeach
              </tbody>
            </table>
          </div>
